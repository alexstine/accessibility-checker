...
const packageScannerConfig = {
    entry: {
        //scanner: path.resolve(__dirname, 'src/scanner/exposed-apis.ts')
        scanner: path.resolve(__dirname, 'src/scanner/custom-rule-configurations.ts')
    ...
    devtool: false,
    output: {
        path: path.join(__dirname, 'packages/scanner/bundle'),
        filename: '[name].bundle.js',
        pathinfo: false,
        library: '[name]',
        libraryTarget: 'umd',
...
const packageTestConfig = {
    entry: {
        test: path.resolve(__dirname, 'src/injected/client-init.ts')
    },
...
module.exports = [packageMsRulesConfig,];
